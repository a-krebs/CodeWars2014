			...
			try
			{
				...
				switch (status)
				{
					...
					case PlayerAIBase.STATUS.PASSENGER_REFUSED_ENEMY:
                        ptDest = Companies.Where(cpy => cpy != Me.Limo.Passenger.Destination && cpy.Passengers.Intersect(Me.Limo.Passenger.Enemies).Count() == 0).OrderBy(cpy =>  CalculatePathPlus1(Me, cpy.BusStop).Count()).First().BusStop;
						break;
